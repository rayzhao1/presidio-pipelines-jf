...
if __name__ == '__main__':
    ...
    h5_files = [fn for fn in input_dir_files if 'preprocess' not in fn]
    ...
    for h5_fn in h5_files:
        ...
        # Preprocess
        print(f"Preprocessing file {h5_fn} by process {os.getpid()}...")
        preprocess_fn = prespipe.ieeg.stage_one_preprocess.Pipeline(h5_fn, output_dir)
        preprocess_success = preprocess_fn in glob(os.path.join(output_dir, '*'))
        print(f"Success? ", preprocess_success)

        # Wavelet transform
        print(f"Performing wavelet transform on {preprocess_fn} by process {os.getpid()}")
        ...
        # Wavelet means
        print(f"Computing wavelet means on {wavelettransform_fn} by process {os.getpid()}")
