...
def map_fn(h5_fn: str, output_dir: str, edf_meta_csv: str, sleep_stages_dict):
    def get_states_file(time):
        for range in sleep_stages_dict:
    ...
    np.savez(sleep_state_aligned_fn, waveletpower_arr=waveletpower_arr, sleep_states_arr=sleep_states_arr, times_artificial=_, time_h5=times_h5)
...
if __name__ == '__main__':
    ...
    sleep_stages_dir_files = glob(sleep_stages_dir)
    ...
    assert len(night_time_ranges) == len(sleep_stages_dir_files)
    ...
    with (multiprocessing.Manager() as manager):
        ...
        for night_num in range(len(sleep_stages_dir_files)):
            fn = os.path.join(sleep_stages_dir, f'PR05_night_{night_num}.1 Stages')
