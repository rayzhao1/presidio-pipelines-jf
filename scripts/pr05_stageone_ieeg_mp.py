...
def map_fn(h5_fn: str, output_dir: str, edf_meta_csv: str, sleep_stages_txt):
    ...
    waveletpower_arr, sleep_states_arr = prespipe.ieeg.sleep_stage_align.Pipeline(waveletpower_fn, sleep_stages, edf_meta)
    sleep_state_aligned_fn = f"{waveletpower_fn.split('.')[0]}.npz"
    np.savez(sleep_state_aligned_fn, waveletpower_arr=waveletpower_arr, sleep_states_arr=sleep_states_arr)
