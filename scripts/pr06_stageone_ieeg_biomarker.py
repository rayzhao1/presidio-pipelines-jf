...
if __name__ == '__main__':
    ...
    df_sxsurv['survey_start'] = pd.to_datetime(df_sxsurv['SurveyStart'])
    print(df_sxsurv.columns)

    """
    #
    for score_type in ['VAS_Depression']:
    ...
    # Remove bad trials
    bad_trials_idx = [24,33,34]
    all_trials_idx = np.arange(h5_powers.shape[0])
    gd_trials_idx = np.setdiff1d(all_trials_idx, bad_trials_idx)
    h5_powers = h5_powers[gd_trials_idx]
    df_sxsurv = df_sxsurv.iloc[gd_trials_idx]
    ...
    for fq in range(corrs.shape[0]):
        for ch in range(corrs.shape[1]):
            corrs[fq, ch], pvals[fq, ch] = sp_stats.pearsonr(h5_powers[:,fq,0,ch], df_sxsurv['VAS_Depression'])
            #corrs[fq, ch], pvals[fq, ch] = sp_stats.spearmanr(h5_powers[:,fq,0,ch], df_sxsurv['VAS_Depression'])
    ...
    ax = plt.subplot(111)
    ax = sns.heatmap(df_corrs, vmin=-0.5, vmax=0.5, cmap='RdBu_r', cbar_kws={'label': 'PearsonR'}, ax=ax)
    ...
    df_corrs_thresh[df_pvals > 0.05] = np.nan
    plt.figure(figsize=(9,9))
    ax = plt.subplot(111)
    ax = sns.heatmap(df_corrs_thresh, vmin=-0.5, vmax=0.5, cmap='RdBu_r', cbar_kws={'label': 'PearsonR'}, ax=ax)
    ax.set_xticks(np.log10(freq_list) * 50/df_corrs.columns.max())
    ax.set_xticklabels(freq_list)
    ax.set_yticks(ch_idx)
    ax.set_yticklabels(ch_grp)
    ...
    for chan in ['vLA', 'vLH', 'vRA', 'vRH']:
        ...
        for ii, lbl in enumerate(chan_lbl):
            ...
            ax.scatter(np.log10(h5_powers[:, :, :, :][:, band_ix, 0, ii].mean(axis=1)), df_sxsurv['VAS_Depression'], s=5)
            rv, pv = sp_stats.pearsonr(np.log10(h5_powers[:, band_ix, 0, ii].mean(axis=1)), df_sxsurv['VAS_Depression'])
            ax.set_xlabel('log Spectral Power (delta band)')
