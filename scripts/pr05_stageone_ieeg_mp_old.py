...
def map_fn(h5_fn: str, output_dir: str, edf_meta_csv: str, sleep_stages_txt):
    ...
    sleep_state_aligned_df = prespipe.ieeg.sleep_stage_align.Pipeline(waveletpower_fn, sleep_stages, edf_meta)
    sleep_state_aligned_fn = f"{waveletpower_fn.split('.')[0]}.pkl"
    sleep_state_aligned_df.to_pickle(sleep_state_aligned_fn)
...
if __name__ == '__main__':
    ...
    h5_files = [(fn, output_dir, edf_meta_csv, sleep_stages_txt) for fn in input_dir_files if 'preprocess' not in fn][:2]
